...
class File{
     ...
     * @param string $ext
     */
    static public function download($file_path,$file_name = '',$file_size = '',$ext=''){
        if (!$file_name){
...
//                return $e->getMessage();
     ...
     */
    static public function downloadImage($url,$filename,$type=0){
        if($url=='')
            return false;
...
//        $size=strlen($img);
     ...
     * @return string
     */
    static public function getMime($ext){
        $mimes = [
            'xml'  => 'text/xml',
